...
void error_number(char*flagname,char*number,int language)
{
	...
	if(ile!=dl)
	{
		...
		if(language==3)printf("Apres %s tu as écrit %s. Ce n'est pas un chiffre!!\n",flagname,number);
		if(language==4)printf("After %s Hai scritto %s. Non e un numero!!\n",flagname,number);
		if(language==5)printf("Después de la %s que escribiste %s. No es un numero!!\n",flagname,number);
...
void error_number_size(char*name,int number,int min, int mak,int language)
{
	if(number<min||number>mak)
	{
		...
		if(language==3)printf("En %s nombre n\'est pas un élément de l\'intervalle <%d,%d>!!\n",name,min,mak);
...
void error_falgargument(char*flagname,int language)
{
		...
		if(language==5)printf("Después de la %s No has introducido un valor!!\n",flagname);
