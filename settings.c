...
//stworzenie zmiennej globalnej wskazującej język i ustawienie jej jako wartość domyślna (j. angielski)
L language=en;
...
//sprawdzenie czy użytkownik podał wymiary planszy oraz ilość generacji
...
//sprawdzenie czy użytkownik podał ustawienia grafiki: rozmiar pikseli, kolory żywych i martwych komórek
...
//sprawdzenie czy użytkownik podał nazwy plików: z danymi oraz wynikowych
void check_filenames(char**filename,SET*setting_pointer,char*argv[],int argc)
{
	int is_filename=0;//zmienna pomocnicza, która wskazuje czy został podany plik
	...
	if(is_filename==0) error_nofilename();//sprawdzenie czy użytkownik podał plik z danymi
}
