...
void check_gridsize(int*x,int*y,int*N,char*argv[],int argc,int language)
{
		for(int i=1;i<argc;i++)
	{
		...
		if(strcmp(argv[i],"--N")==0) {if(i+1>=argc)error_falgargument("--N",language);error_number("--N",argv[i+1],language);*N=atoi(argv[i+1]);}
