...
void check_gridsize(int*x,int*y,int*generation_number,char*argv[],int argc)
{
	...
	{
		if(strcmp(argv[i],"--x")==0) {if(i+1>=argc)error_falgargument("--x");error_number("--x",argv[i+1]);error_number_size("--x",atoi(argv[i+1]),1,100);*x=atoi(argv[i+1]);}
		if(strcmp(argv[i],"--y")==0) {if(i+1>=argc)error_falgargument("--y");error_number("--y",argv[i+1]);error_number_size("--y",atoi(argv[i+1]),1,100);*y=atoi(argv[i+1]);}
		if(strcmp(argv[i],"--N")==0) {if(i+1>=argc)error_falgargument("--N");error_number("--N",argv[i+1]);*generation_number=atoi(argv[i+1]);}
	}
