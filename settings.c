...
void check_gridsize(int*x,int*y,char*argv[],int argc,int language)
{
		for(int i=1;i<argc;i++)
	{
		...
		if(strcmp(argv[i],"--y")==0) {error_number("--y",argv[i+1],language);error_number_size("--y",atoi(argv[i+1]),1,100,language);*y=atoi(argv[i+1]);}
	}
...
void check_filenames(char**filename,char**graphicfile,char**txtfile,int*is_filename,int*is_graphicname,int*is_txtname,char*argv[],int argc)
{
	for(int i=1;i<argc;i++)
	{
